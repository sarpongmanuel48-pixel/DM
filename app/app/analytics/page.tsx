...
/**
 * Standalone analogue of app/dashboard/[companyId]/analytics/page.tsx.
 * The one real content difference: the Whop path's "Handoffs to Whop"
 * stat is specific to Whop checkout — for a standalone creator (whose
 * offers, once that exists, could hand off anywhere), the generic
 * "Offer handoffs" label is what's actually accurate, not a cosmetic
 * change.
 */
export default async function StandaloneAnalyticsPage() {
    ...
    <div className="flex flex-col gap-5">
      ...
      <div className="grid grid-cols-[1.4fr_1fr] gap-3">
        ...
        <div className="flex flex-col justify-center gap-1 rounded-lg border border-hairline bg-white p-5">
          <div className="text-[10px] font-semibold uppercase tracking-widest text-ink-500">Offer handoffs</div>
          <div className="font-display text-[32px] font-bold tracking-tight text-ink-900">{handoffs}</div>
          <p className="text-xs text-ink-500">
            Taps that reached an offer&apos;s checkout link — not confirmed sales.
          </p>
        </div>
      </div>
